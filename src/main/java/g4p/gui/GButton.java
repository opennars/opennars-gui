...
import g4p.gui.HotSpot.HSrect;
import g4p.gui.StyledString.TextLayoutInfo;
 ...
 *
 * To simplify event handling the button only fires off CLICKED events if the
 * mouse button is pressed and released over the button face (the default
 * behaviour). <br>
 *
 * Using
 * <pre>button1.fireAllEvents(true);</pre> enables the other 2 events for button
 * <b>button1</b>. A PRESSED event is created if the mouse button is pressed
 * down over the button face, the CLICKED event is then generated if the mouse
 * button is released over the button face. Releasing the button off the button
 * face creates a RELEASED event. <br>
 *
...
public class GButton extends GTextIconAlignBase {
     ...
     * the mouse off button image is used.
     *
    ...
    /**
     *
     * When a mouse button is clicked on a GButton it generates the
     ...
     * </pre> <br>
     * Where
     * <pre><b>btnName</b></pre> is the GButton identifier (variable name)
     * <br><br>
     *
     */
